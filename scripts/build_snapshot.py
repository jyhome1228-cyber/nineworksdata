...
HEADERS = {
    ...
    'User-Agent': '9w-control-snapshot',
}
if TOKEN:
...
def fetch_project(repo):
    ...
    paths = {str(x.get('path', '')).lower(): x for x in tree}
    ...
    cname = decode_content(cname_obj).strip().split()[0] if decode_content(cname_obj).strip() else ''
    ...
    enriched = dict(repo)
    enriched['_snapshot'] = {
        'readme_summary': snapshot['readme_summary'],
        'cname': cname,
        'commits': compact_commits,
        'tree_count': len(tree),
